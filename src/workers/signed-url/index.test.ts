  ...
  describe('POST /generate', () => {
    it('should generate signed URL with default values', async () => {
      ...
      expect(result.url).toMatch(
        /https:\/\/example\.com\/download\/export-\d+\.csv\?expires=\d+&token=[a-f0-9]+/
      );
      expect(mockBucket.put).toHaveBeenCalledWith(
        expect.stringMatching(/export-\d+\.csv/),
        'a,b,c\\n1,2,3'
      );
    });
  ...
  describe('GET /download', () => {
    ...
    it('should return 403 for expired token', async () => {
      ...
      const request = new Request(
        `https://example.com/download/test.csv?expires=${expiredTime}&token=invalid`
      );
    ...
    it('should return 403 for invalid token', async () => {
      ...
      const request = new Request(
        `https://example.com/download/test.csv?expires=${futureTime}&token=invalid`
      );
    ...
    it('should return 404 for non-existent file', async () => {
      ...
      const request = new Request(
        `https://example.com/download/${key}?expires=${expires}&token=${token}`
      );
      const response = await worker.fetch(request, mockEnv);
