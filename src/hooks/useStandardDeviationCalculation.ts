/**
 * Hook for standard deviation calculations with comprehensive statistical analysis
 * Supports both sample and population calculations, outlier detection, and educational features
 */
...
export const useStandardDeviationCalculation = (): UseStandardDeviationCalculation => {
  ...
  const generateCalculationSteps = useCallback((
    ...
    steps.push(`Step 7: Calculate standard deviation: σ = √${variance.toFixed(4)} = ${Math.sqrt(variance).toFixed(4)}`);
    
    return steps;
  }, []);

  const validateData = useCallback((points: DataPoint[]) => {
    const errors: string[] = [];
  ...
  const calculate = useCallback(async (
    points: DataPoint[], 
    options: Partial<StandardDeviationCalculatorState> = {}
  ) => {
    ...
    try {
      ...
      // Advanced statistics
      const skewness = calculateSkewness(cleanValues, mean, sampleStdDev);
      const kurtosis = calculateKurtosis(cleanValues, mean, sampleStdDev);
      const coefficientOfVariation = mean !== 0 ? (sampleStdDev / Math.abs(mean)) * 100 : 0;
      const standardError = count > 1 ? sampleStdDev / Math.sqrt(count) : 0;
      ...
      const result: StandardDeviationResult = {
        ...
        populationStandardDeviation: populationStdDev,
        variance: isPopulation ? populationVariance : sampleVariance,
        sampleVariance,
        ...
        // Metadata
        calculationType: calculationType as 'sample' | 'population' | 'both',
      ...
      const errorMessage = err instanceof StandardDeviationError ? 
  ...
  const updateDataPoint = useCallback((id: string, updates: Partial<DataPoint>) => {
...
export default useStandardDeviationCalculation;